...
public partial class ReadMeTest
{
    ...
    // Navigate from src/Maple.Native.DocTest/ up to repo root (2 levels: DocTest → src → root)
    ...
    {
        // [CallerFilePath] resolves to a deterministic path at compile time.
        // On CI runners the repo root may differ — skip if README not found.
    ...
    public void ReadMeTest_UpdateBenchmarksInMarkdown()
    {
        ...
        foreach (var (fileName, config) in benchmarkFileNameToConfig)
        {
            ...
            {
                continue;
            } // no benchmark data yet — skip embedding this entry
    ...
    public void ReadMeTest_PublicApi()
    {
        ...
        var publicApi = typeof(Native).Assembly.GeneratePublicApi();
